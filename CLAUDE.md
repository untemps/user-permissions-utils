...
- Chaque point d'entrée vérifie **en ligne** l'existence de l'API navigateur (`!navigator.permissions`) et lève `NotSupportedError` si elle est absente — **sauf `getUserMediaStream`**, qui ne garde que `!navigator.mediaDevices` et traite la Permissions API comme best-effort (voir plus bas). Détection de support côté consommateur : la lib exporte **deux** prédicats `is…Supported`, un par API navigateur encapsulée — `isPermissionsSupported` (présence de l'**API Permissions**) et `isMediaDevicesSupported` (présence de **`mediaDevices`**). Ils détectent la **présence d'API** (porte de précondition synchrone) ; pour lire un **état** de permission (`granted`/`denied`/`prompt`), c'est `checkPermission` (async, qui rejette/propage l'erreur de `query()`). Les deux répondent à des questions différentes : `checkPermission` est un mauvais proxy de présence (un nom non interrogeable — `camera`/`microphone` sur Firefox — lève un `TypeError` alors que `navigator.permissions` est bien présent) et ne couvre de toute façon pas `mediaDevices` (il sonde `navigator.permissions`, pas `navigator.mediaDevices`).
- `isMediaDevicesSupported()` / `isPermissionsSupported()` : prédicats **purs, synchrones, sans effet de bord, SSR-safe** (`(): boolean`), strictement symétriques, qui retournent respectivement `!!globalThis.navigator?.mediaDevices?.getUserMedia` et `!!globalThis.navigator?.permissions?.query` (test sur `.query`, la méthode réellement appelée par la lib, pas la seule présence de l'objet `permissions`). Ne déclenchent aucun prompt, ne lèvent jamais (le chaînage optionnel via `globalThis.navigator?.…` évite toute `ReferenceError` côté SSR). `isPermissionsSupported` ne se substitue **pas** à `checkPermission` : il détecte la **présence** de l'API, là où `checkPermission` lit un **état** (asynchrone par nature — l'état effectif vit hors du renderer : processus navigateur, stockage persistant, Permissions Policy).
